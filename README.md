...
Enumpath is an implementation of the JSONPath spec for Ruby objects, plus some added sugar. It's like Ruby's native `dig` method, but fancier. It is designed for situations where you need to dynamically describe a path through complex nested enumerable objects. This makes it exceptionally well suited for flexible ETL (Extract, Transform, Load) processes by allowing you to define paths through your data in a simple, readable, easily storable fashion.
...
Enumpath exposes a simple interface via `.apply` that takes a path and an enumerable.
...
The **result** of `.apply` is an array of values that were extracted according to the path. Technically it's an instance of `Enumpath::Results` which quacks like an Array but allows you to do things like this:

```ruby
party = { food: %w(pizza tacos) }
Enumpath.apply('food.*', party).apply("[?(@ == 'pizza')]") # => ["pizza"]
```
...
### Returning paths instead of values
...
```ruby
party = { food: %w(pizza tacos) }
Enumpath.apply("food.*", party, result_type: :path)
# => ["$['food'][0]", "$['food'][1]"]
```
...
`.` or `[]` | **Child**; can be dot notation or bracket, and bracketed child operators can optionally be wrapped in single quotes | `locations`, `0`, `[departments]`, and `['human resources']` are all valid child operators
`*` | **Wildcard**; flattens the current enumerable | `children.*.name` would result in an array containing the names of all the children
`..` | **Recursive descent**; applies the remaining segments all the way down the enumerable, including the current enumerable, until they are fully resolved | `..name` would find all the `name` nodes all the way down through the enumerable regardless of nesting level
`[start:end:step]` | **Slice**; similar in functionality to Ruby's `Array#slice` method with the addition of a step argument; at least one of the arguments is required | `[1:8:2]` would operate on indices 1, 3, 5, and 7. See the [examples](#examples) section below for further usage examples
`[child1,child2[,...]]` | **Union**; combines the results from multiple child operators | `authors.*[first_name,last_name]` is equivalent to the combined results of `authors.*.first_name` and `authors.*.last_name`
`?()` | **Filter expression**; evaluates boolean expressions against the current enumerable; only the nodes of enumerable that meet the criteria are passed through; expressions can include comparison operators; multiple expressions can be chained together using logical operators | `[?(@.price > 10 && @.price <= 20 )]` would return all items whose price is greater than 10 and less than or equal to 20
`()` | **Subscript expression**; evaluates an expression and applies it as a child operator on the current enumerator | `[(@.length - 1)]` would apply a child operator equal to the `#length` of the current enumerable minus 1 to the current enumerable (i.e. the last member of an array)
...
Enumpath.apply("$..book[:2]", store_info)
# => Both return: [{:category=>"reference", :author=>"Nigel Rees", :title=>"Sayings of the Century", :price=>8.95}, {:category=>"fiction", :author=>"Evelyn Waugh", :title=>"Sword of Honour", :price=>12.99}]
...
The slice operator selects a range of elements like Ruby's _`start...end`_ literal, excluding the end value, and then selects each _step_ items. The _start_, _end_, and _step_ arguments default to `0`, `Enumerable#length`, and `1` respectively.

The operator accepts a mixed bag of argument combinations. For instance, these are all valid slice operators:

- `[1:8]` == indices 1 – 7
- `[1:]` == indices 1 – `Enumerable#length`
- `[:8]` == indices 0 – 7
- `[:8:2]` == indices 0, 2, 4, and 6
- `[::2]` == indices 0, 2, 4, 6, 8, ...
...
A subscript expression is made up of a singe expression that consists of a child operator (`@.child` (or `child`; the leading `@.` is optional)), plus an optional pair of arithmetic operator and operand. The arithmetic operator can be any one of `+`, `-`, `**`, `*`, `/`, or `%`. The operand can be a string (`'some string'`), symbol (`:some_symbol`), or numeric value (`10` or `1.0`). If an operator and operand are not included in an expression then the value located at the child operator is used as-is. If the result of the expression represents a valid child path for the enumerable, the value of the child member will be passed along to the next processing step.

The following are all valid filter expressions:

- `[(@.length - 1)]` == the length of the current enumerable, minus 1
- `[(length - 1)]` == equivalent to the previous example
- `[(@.type)]` == the value at `type`

## More About Path Expressions
...
Enumpath will attempt to resolve the data type of each path segment in the following order of precedence:

1. as an integer key or index (if the segment is integer-like),
2. then as a string key,
3. then as a symbol key,
4. and finally as a public property (i.e. a public method of the target that accepts no arguments)

## Deviations from the Original JSONPath Spec

1. Whereas the JSONPath spec required that `false` be returned when no matches were found, Enumpath will return an empty result set (`[]`) instead. This is a thoughtful divergence based on the principle of least astonishment and the robustness principle.
...
3. Whereas the original implementations of JSONPath allowed unsafe evaluation of filter and subscript expressions, Enumpath limits those expressions to a reasonable subset of operations as detailed above and uses `public_send` rather than `eval` to resolve expressions when necessary.

4. The original JSONSpec path did not include support for using logical operators to chain expressions in filter expression operators. This addition was inspired by [Gergely Brautigam's](https://skarlso.github.io/2017/05/28/replace-eval-with-object-send-and-a-parser/) work on [joshbuddy/jsonpath](https://github.com/joshbuddy/jsonpath)
